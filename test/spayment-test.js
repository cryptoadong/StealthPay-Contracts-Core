...
describe('StealthPay', () => {
  ...
  before(async () => {
    ...
    ctx.StealthPay = await StealthPay.new(deployedToll, tollCollector, tollReceiver, { from: owner });
  ...
  it('should have correct values initialized', async () => {
    const theOwner = await ctx.StealthPay.owner();
    expect(theOwner).to.equal(owner);

    const theCollector = await ctx.StealthPay.tollCollector();
    expect(theCollector).to.equal(tollCollector);

    const theReceiver = await ctx.StealthPay.tollReceiver();
  ...
  it('should let the owner update the toll', async () => {
    await ctx.StealthPay.setToll(updatedToll, { from: owner });
    const toll = await ctx.StealthPay.toll();
  ...
  it('should not allow someone other than the owner to update the toll', async () => {
    await expectRevert(ctx.StealthPay.setToll(deployedToll, { from: other }), 'Ownable: caller is not the owner');
  });
  ...
  it('should not allow someone to pay less than the toll amount', async () => {
    ...
    await expectRevert(
      ctx.StealthPay.sendEth(receiver1, toll, ...argumentBytes, { from: payer1, value: paymentAmount }),
      'StealthPay: Must pay more than the toll'
    );
  });
  ...
  it('should not allow someone to pay exactly the toll amount', async () => {
    ...
    await expectRevert(
      ctx.StealthPay.sendEth(receiver1, toll, ...argumentBytes, { from: payer1, value: toll }),
      'StealthPay: Must pay more than the toll'
    );
  });
  ...
  it('should not allow an eth payment that commits to the wrong toll', async () => {
    await expectRevert(
      ctx.StealthPay.sendEth(receiver1, deployedToll, ...argumentBytes, {
        from: payer1,
        value: ethPayment,
      }),
      'StealthPay: Invalid or outdated toll commitment'
    );
  });
  ...
  it('should allow someone to pay in eth', async () => {
    ...
    const receipt = await ctx.StealthPay.sendEth(receiver1, toll, ...argumentBytes, {
      from: payer1,
      value: ethPayment,
    });
  ...
  it('should not let the eth receiver withdraw tokens', async () => {
    await expectRevert(
      ctx.StealthPay.withdrawToken(acceptor, ctx.token.address, { from: receiver1 }),
      'StealthPay: No balance to withdraw or fee exceeds balance'
    );
  });

  it('should not allow someone to pay with a token without sending the toll', async () => {
    await expectRevert(
      ctx.StealthPay.sendToken(receiver2, ctx.token.address, tokenAmount, ...argumentBytes, {
        from: payer2,
      }),
      'StealthPay: Must pay the exact toll'
    );
  });
  ...
  it('should allow someone to pay with a token', async () => {
    const toll = await ctx.StealthPay.toll();
    await ctx.token.approve(ctx.StealthPay.address, tokenAmount, { from: payer2 });
    const receipt = await ctx.StealthPay.sendToken(receiver2, ctx.token.address, tokenAmount, ...argumentBytes, {
      from: payer2,
      value: toll,
    });

    const contractBalance = await ctx.token.balanceOf(ctx.StealthPay.address);
  ...
  it('should allow someone to pay tokens to a previous ETH receiver', async () => {
    const toll = await ctx.StealthPay.toll();
    await ctx.token.approve(ctx.StealthPay.address, secondTokenAmount, { from: payer2 });
    const receipt = await ctx.StealthPay.sendToken(receiver1, ctx.token.address, secondTokenAmount, ...argumentBytes, {
      from: payer2,
      value: toll,
    });
  ...
  it('should not allow someone to pay a token to a reused address', async () => {
    ...
    await expectRevert(
      ...
      'StealthPay: Cannot send more tokens to stealth address'
    );
  });
  ...
  it('should allow someone to send ETH to a previous Token receiver', async () => {
    ...
    const receipt = await ctx.StealthPay.sendEth(receiver2, toll, ...argumentBytes, {
      from: payer1,
      value: secondEthPayment,
    });
  ...
  it('should not allow a non-receiver to withdraw tokens', async () => {
    await expectRevert(
      ctx.StealthPay.withdrawToken(acceptor, ctx.token.address, { from: other }),
      'StealthPay: No balance to withdraw or fee exceeds balance'
    );
  });

  it('should allow receiver to withdraw their token', async () => {
    const receipt = await ctx.StealthPay.withdrawToken(acceptor, ctx.token.address, { from: receiver2 });
  ...
  it('should not allow a receiver to withdraw their tokens twice', async () => {
    await expectRevert(
      ctx.StealthPay.withdrawToken(acceptor, ctx.token.address, { from: receiver2 }),
      'StealthPay: No balance to withdraw or fee exceeds balance'
    );
  });
  ...
  it('should revert on a meta withdrawal when the stealth address does not have a balance', async () => {
    ...
    await expectRevert(
      ctx.StealthPay.withdrawTokenOnBehalf(
        metaWallet.address,
        metaAcceptor,
        ctx.token.address,
        relayer,
  ...
  it('should revert on meta withdrawal signed by the wrong address', async () => {
    // Send the payment
    const toll = await ctx.StealthPay.toll();
    await ctx.token.approve(ctx.StealthPay.address, metaTokenTotal, { from: payer2 });
    await ctx.StealthPay.sendToken(metaWallet.address, ctx.token.address, metaTokenTotal, ...argumentBytes, {
      from: payer2,
      value: toll,
    });
  ...
  it('should revert on meta withdrawal if the fee is more than the amount', async () => {
    ...
    await expectRevert(
      ctx.StealthPay.withdrawTokenOnBehalf(metaWallet.address, metaAcceptor, ctx.token.address, relayer, bigFee, v, r, s, {
        from: relayer,
      }),
      'StealthPay: No balance to withdraw or fee exceeds balance'
    );
  ...
  it('perform a withdrawal when given a properly signed meta-tx', async () => {
    ...
    const receipt = await ctx.StealthPay.withdrawTokenOnBehalf(
      metaWallet.address,
      metaAcceptor,
      ctx.token.address,
      relayer,
  ...
  it('should not allow someone else to move tolls to toll receiver', async () => {
    await expectRevert(ctx.StealthPay.collectTolls({ from: other }), 'StealthPay: Not toll collector');
  });
  ...
  it('should allow the toll collector to move tolls to toll receiver', async () => {
    ...
    await ctx.StealthPay.collectTolls({ from: tollCollector });
