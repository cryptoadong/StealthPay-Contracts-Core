/**
 * @notice StealthPay Deployment script
 * Deploys the core StealthPay contract, along with the StealthPay Stealth Key Registry, checkilng for the correct
 ...
 */
 ...
 //const fileName = `${folderName}/${network}-${now}.json`; old
 ...
 (async function () {
   try {
     ...
     console.log('deployerWallet.address: ', deployerWallet.address);
     ...
     if (deployerNonce !== 0) {
       throw new Error('Unexpected non-zero nonce before deploying StealthPay');
     }
 
 
     // deploy the StealthPay contract
     const StealthPay = await ethers.getContractFactory('StealthPay', deployerWallet);
     
     const stealthpay = await StealthPay.deploy(toll, tollCollector, tollReceiver);
     const deployed=await stealthpay.deployed();
 
     console.log('deployed: ', deployed);
 
 
     save(stealthpay.address, 'contracts', 'StealthPay');
     console.log('StealthPay contract deployed to address: ', stealthpay.address);
